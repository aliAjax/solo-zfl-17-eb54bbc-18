...
- **旧数据迁移**：升级前没有 origin 的台账，载入时按台账条目标识跨工单比对——共享记录归最早实际领用的工单（已合并的顺延给合并单），副本独有的记录仍归副本；旧版合并单的台账按"来源工单原始长度"切分：前段是来源拼接、按条目 id 去重还原，尾段是合并后新领、全部保留（同工序同物料同数量的多笔互不影响）；连续合并按建单时间先重建上游再重建下游。迁移只补登归属，不改库存数字。
